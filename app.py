...
# st.markdown("""
# <style>
# /* ── NUCLEAR BLUR KILL — covers every widget interaction ── */
# *, *::before, *::after {
#     backdrop-filter: none !important;
#     -webkit-backdrop-filter: none !important;
# }
...
# /* BaseWeb popover / menu / select / input */
# [data-baseweb="popover"], [data-baseweb="popover"] *,
# [data-baseweb="menu"], [data-baseweb="menu"] *,
# [data-baseweb="select"], [data-baseweb="select"] *,
# [data-baseweb="input"], [data-baseweb="input"] *,
# [data-baseweb="base-input"], [data-baseweb="base-input"] *,
# [data-baseweb="checkbox"], [data-baseweb="checkbox"] *,
# [data-baseweb="slider"], [data-baseweb="slider"] *,
...
# /* Number input container — the blur fires here on click */
...
# /* Select slider (the coloured track widget) */
...
# div[style*="backdrop-filter"], div[style*="backdropFilter"],
# div[style*="blur("], span[style*="blur("],
# button[style*="blur("], input[style*="blur("] {
#     backdrop-filter: none !important;
#     -webkit-backdrop-filter: none !important;
#     filter: none !important;
# }

# /* Force all buttons — including stepper +/- — to render cleanly */
# button {
#     backdrop-filter: none !important;
#     -webkit-backdrop-filter: none !important;
#     filter: none !important;
...
#     /* Also directly target number input stepper buttons by data-testid */
...
#             killBlur(el);
#             /* Also remove computed filter from stylesheet */
#             el.style.filter = 'none';
#             el.style.backdropFilter = 'none';
#             el.style.webkitBackdropFilter = 'none';
#             el.style.transition = 'background 0.15s ease, box-shadow 0.15s ease';
#         });
#     }
...
#     /* Extended event list — blur fires on mouseenter/leave and pointerup for steppers */
...
#     /* Faster poll — 20ms catches the stepper hover flash */
#     setInterval(function() { scanAll(); killSteppers(); }, 20);
# })();
# </script>
# """, unsafe_allow_html=True)
...
# /* Number input stepper +/- buttons — kill blur on ALL states */
# [data-testid="stNumberInput"] button,
# [data-testid="stNumberInputStepDown"],
# [data-testid="stNumberInputStepUp"] {
#     backdrop-filter: none !important;
...
# # ─────────────────────────────────────────────────────────────────────────────
# # CSS / JS  —  COMPLETE BLUR KILL + DESIGN SYSTEM
# # ─────────────────────────────────────────────────────────────────────────────

# st.markdown("""
# <style>
# /* ═══════════════════════════════════════════════════════
#    NUCLEAR BLUR KILL — every widget, every state
#    ═══════════════════════════════════════════════════════ */
...
# /* Number input stepper +/- buttons — ALL states */
# [data-testid="stNumberInput"],
# [data-testid="stNumberInput"] > div,
# [data-testid="stNumberInput"] > div > div,
# [data-testid="stNumberInput"] > div > div > div,
...
@st.cache_data(show_spinner=False)   # ← read gif from disk only once
def _gif_b64() -> str:
    if os.path.exists(_GIF_PATH):
        with open(_GIF_PATH, "rb") as f:
            return base64.b64encode(f.read()).decode()
...
def show_loader(placeholder, message: str = "Processing…"):
    """
    Renders a loading animation into `placeholder` (an st.empty()).
    Uses a container inside the placeholder so clearing it is instant
    and never causes a full-page grey-out.
    """
    b64 = _gif_b64()
    ...
    with center:
        ...
        if tab == "🔑  Login":
            ...
            password = st.text_input("🔒  Password", placeholder="Enter your password",
                                     type="password")
            st.markdown("<div style='height:0.4rem'></div>", unsafe_allow_html=True)
            # Use st.empty for error so only this slot updates, not the whole page
            _login_err = st.empty()
            if st.button("Login  →", width='stretch'):
                if login_user(username, password):
                    st.session_state.logged_in = True
                    ...
                    _login_err.error("❌ Invalid username or password.")
        else:
            st.markdown("""
            <div class="rule-box">
                🔐 <b>Password must have:</b><br>
...
#   • on_click callbacks  → no inline st.rerun() inside sidebar buttons
#   • @st.fragment        → sidebar re-renders independently; main page
#                           does NOT grey-out when a nav button is clicked
#   • st.cache_data       → logo b64 cached across reruns
# ─────────────────────────────────────────────────────────────────────────────
...
# ── on_click callback helpers (set state, no manual st.rerun needed) ─────────

def _nav_to(page: str):
    """Navigate to a page. Called via on_click — Streamlit auto-reruns."""
...
# ── Cache the logo so it is not re-read from disk on every rerun ─────────────

@st.cache_data(show_spinner=False)
def _cached_logo_b64() -> str:
    return _sidebar_logo_b64()

@st.fragment          # ← fragment: only this block re-runs on nav clicks
def _render_sidebar(username: str, role: str):
    """Sidebar rendered as a fragment — main page stays live (no grey-out)."""
    _logo_b64 = _cached_logo_b64()
    _logo_html = (
        "<div style='text-align:center;margin-bottom:0.3rem;padding-top:0.8rem;'>"
        "<img src='data:image/avif;base64," + _logo_b64 + "'"
        " style='width:80px;height:80px;object-fit:contain;border-radius:14px;'>"
    ...
    # Home button — on_click sets state; Streamlit does a targeted rerun
    st.markdown("<div class='home-btn'>", unsafe_allow_html=True)
    st.button(
        "🏠  Home", key="home_btn", width="stretch",
        on_click=_nav_to, args=("About",),
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    for icon, label, _tech in _feature_list:
        if label == "Model Insights" and role != "admin":
            continue
        _cls = "nav-btn-active" if _current_menu == label else "nav-btn"
        st.markdown(f"<div class='{_cls}'>", unsafe_allow_html=True)
        # on_click: sets session_state.menu then Streamlit auto-reruns —
        # NO manual st.rerun() means the grey-out flash is eliminated.
        st.button(
            f"{icon}  {label}", key=f"nav_{label}", width="stretch",
            on_click=_nav_to, args=(label,),
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if role == "admin":
        ...
        _acls = "admin-btn-active" if _current_menu == "Admin Panel" else "admin-btn"
        st.markdown(f"<div class='{_acls}'>", unsafe_allow_html=True)
        st.button(
            "🛡️  Admin Panel", key="nav_admin_panel", width="stretch",
            on_click=_nav_to, args=("Admin Panel",),
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='logout-btn'>", unsafe_allow_html=True)
    st.button(
        "🚪  Logout", key="logout_btn", width="stretch",
        on_click=_logout_cb,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
with st.sidebar:
    _render_sidebar(username, role)


# ─────────────────────────────────────────────────────────────────────────────
# PAGE HEADER  (fragment → only re-renders when menu changes)
# ─────────────────────────────────────────────────────────────────────────────
...
@st.cache_data(show_spinner=False)
def _hero_img_b64() -> str:
    path = "assets/robot_hand.jpg"
    if os.path.exists(path):
        with open(path, "rb") as f:
            return base64.b64encode(f.read()).decode()
    return ""
