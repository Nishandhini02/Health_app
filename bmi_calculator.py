...
API_KEY = "AIzaSyDKQlD9Fu09Hq4PQid5zvdcVisECJGB04s"

def show_bmi_calculator(show_loader):
    ...
    with col1:
        ...
        with hc1:
            bp        = st.number_input("Blood Pressure (mmHg)", 40, 120, 70,
                                         key="bmi_bp")
            glucose   = st.number_input("Glucose (mg/dL)", 40, 400, 90,
                                         key="bmi_glucose")
        with hc2:
            chol      = st.number_input("Cholesterol (mg/dL)", 100, 400, 180,
                                         key="bmi_chol")
            insulin   = st.number_input("Insulin (μU/mL)", 0, 300, 80,
                                         key="bmi_insulin")
    ...
    with col2:
        if calc_btn:
            ...
            # Blood Pressure (20 pts) — dataset range 31-108
            if bp < 70:                score += 20
            elif bp < 80:              score += 16
            elif bp < 90:              score += 11
            elif bp < 100:             score += 6
            ...
            breakdown = {
                "BMI":           (25, min(25, score)),
                "Blood Pressure":(20, min(20, bp < 70 and 20 or bp < 80 and 16
                                         or bp < 90 and 11 or bp < 100 and 6 or 2)),
                "Glucose":       (20, min(20, glucose < 100 and 20 or glucose < 110
                                         and 16 or glucose < 126 and 10
                                         or glucose < 160 and 5 or 1)),
                "Cholesterol":   (10, min(10, chol < 170 and 10 or chol < 200
                                         and 8 or chol < 240 and 5 or 2)),
            ...
            for label, (max_pts, pts) in breakdown.items():
                ...
                st.markdown(
                    ...
                    f"<span style='color:{bar_col};font-weight:700;width:50px;text-align:right;'>"
                    f"{pts}/{max_pts}</span></div>",
                    unsafe_allow_html=True
                )
            ...
            try:
                ...
                prompt = (
                    ...
                    f"Give 4 specific, actionable health improvement tips in bullet points. "
                    f"Be concise and practical. Focus on the lowest scoring areas."
                )
                tips = model.generate_content(prompt).text.strip()
