...
#     """Strip markdown and return clean UTF-8 text bytes."""
#     import re as _re
#     lines = []
...
#     """
#     English  → PDF via ReportLab (no extra install needed)
#     Others   → clean UTF-8 TXT (perfect Unicode, no font issues)
#     Returns bytes. Caller checks language to decide file type.
#     """
#     import re as _re, os

#     # ── Non-English → TXT immediately, no PDF attempt ────────────────────────
#     if language != "English":
#         return _make_txt(summary_text, language)

#     # ── English → ReportLab PDF ───────────────────────────────────────────────
#     try:
#         import requests
...
#     except Exception:
#         # Any failure → plain TXT fallback even for English
#         return _make_txt(summary_text, language)
...
#     # Language selectbox FIRST — no rerun of heavy OCR when changing language
#     lang = st.selectbox("🌐 Select Language for Summary", [
#         "English","Tamil","Hindi","Telugu","Malayalam",
#         "Kannada","Bengali","Marathi","Gujarati","Urdu"
#     ], key="lab_lang_select")

#     # File uploader SECOND
#     file = st.file_uploader("📁 Upload Report Image", type=["png","jpg","jpeg","webp","jfif"])

#     if file:
#         # Only re-run OCR if a NEW file is uploaded (compare file name+size)
...
#             st.session_state["lab_extracted"]     = extracted_text
#             st.session_state["lab_summary"]       = None  # reset summary on new file

#         # Always show from session state — no OCR rerun on lang change
#         img           = st.session_state.get("lab_img_orig")
#         processed_img = st.session_state.get("lab_img_proc")
#         extracted_text = st.session_state.get("lab_extracted", "")
...
#             with btn_col1:
#                 gen_clicked = st.button("🤖 Generate Medical Summary", width='stretch')
#             with btn_col2:
#                 # Translate only shows if summary already exists
#                 trans_clicked = (
#                     st.button("🌐 Translate Summary", width='stretch')
